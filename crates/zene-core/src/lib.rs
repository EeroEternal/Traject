...
impl Agent {
    pub async fn new(
        ...
        Self::new_with_client(config, sandbox, session, permission_mode, client).await
    }

    /// Build an agent that routes LLM calls through a pre-built client
    /// (e.g. Traject `TrajectLlmProvider`).
